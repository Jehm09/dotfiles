    ...
    function paste(entry) {
        ...
        if (root.cliphistBinary.includes("cliphist")) {
            if (isImage) {
                ...
                Quickshell.execDetached(["bash", "-c",
                    `printf '${StringUtils.shellSingleQuoteEscape(entry)}' | ${root.cliphistBinary} decode | wl-copy && sleep 0.25 && wl-paste | wtype -`
                ]);
            }
        } else { // Stash
            const entryNumber = entry.split("\t")[0];
            Quickshell.execDetached(["bash", "-c",
                `${root.cliphistBinary} decode ${entryNumber} | wl-copy && sleep 0.25 && wl-paste | wtype -`
            ]);
        }
    }
