...
page=$((1 + RANDOM % 1000));
prismConfigPath="$HOME/.config/settings/quickshell/config.json"
userAgent=$(jq -r '.networking.userAgent // empty' "$prismConfigPath" 2>/dev/null)
...
currentWallpaperPath=$(jq -r '.background.wallpaperPath' "$prismConfigPath")
