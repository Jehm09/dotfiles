...
Singleton {
    ...
    Timer {
        ...
        onTriggered: {
            ...
            // CPU temperature (polled via cpuTempPollProc, see below)
            if (!cpuTempPollProc.running) cpuTempPollProc.running = true
    ...
    // Reads from hwmon (AMD k10temp/zenpower Tctl/Tdie, Intel coretemp), falling
    // back to thermal_zone0. Newer AMD kernels drop the ACPI thermal_zone*, so
    // the old fixed /sys/class/thermal/thermal_zone0/temp path returns nothing.
    Process {
        id: cpuTempPollProc
        command: ["bash", "-c", [
            ...
            "  case \"$name\" in",
            "    k10temp|zenpower)",
            "      f=$(grep -il -E 'Tdie|Tctl' \"$d\"/temp*_label 2>/dev/null | head -1);",
            "      [ -n \"$f\" ] && in=\"${f%_label}_input\" || in=\"$d/temp1_input\";",
            "      t=$(cat \"$in\" 2>/dev/null); [ -n \"$t\" ] && { echo \"$t\"; exit; } ;;",
            "    coretemp)",
            "      f=$(grep -il 'Package id 0' \"$d\"/temp*_label 2>/dev/null | head -1);",
            "      [ -n \"$f\" ] && in=\"${f%_label}_input\" || in=\"$d/temp1_input\";",
            "      t=$(cat \"$in\" 2>/dev/null); [ -n \"$t\" ] && { echo \"$t\"; exit; } ;;",
            "  esac;",
            "done;",
        ...
        stdout: StdioCollector {
            id: cpuTempOut
            onStreamFinished: {
                const raw = parseInt(cpuTempOut.text.trim())
                if (!isNaN(raw) && raw > 0) root.cpuTemp = raw / 1000
            }
        }
    }
