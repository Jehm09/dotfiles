...
Singleton {
    ...
    // Kept for the settings/welcome language pickers
    property var allAvailableLanguages: [...root.availableLanguages].sort()
    property bool isScanning: scanLanguagesProcess.running
    property bool isLoading: false
    property string translationKeepSuffix: "/*keep*/"
    property string translationsDir: Quickshell.shellPath("i18n")

    property string languageCode: {
        var configLang = Config?.options.language.ui ?? "en";
    ...
    component TranslationScanner: Process {
        ...
        onExited: (exitCode, exitStatus) => {
            if (exitCode !== 0) {
                translationScanner.languagesScanned(["en"]);
            }
        }
    }
